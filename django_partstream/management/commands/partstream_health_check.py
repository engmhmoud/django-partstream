...
class Command(BaseCommand):
        ...
        try:
            ...
            if not hasattr(settings, 'SECRET_KEY') or not settings.SECRET_KEY:
                return {
                    ...
                    'details': {'issue': 'Missing SECRET_KEY required for cursor encryption'}
                }
    ...
    def _check_cursor_encryption(self):
        ...
        try:
            ...
            return {
                'status': 'passed',
                'message': f'Cursor encryption working in {
                    response_time:.1f}ms',
                'details': {
                    'response_time_ms': response_time,
    ...
    def _check_performance(self):
        ...
        try:
            ...
            return {
                'status': 'passed', 'message': f'Performance monitoring working in {
                    response_time:.1f}ms', 'details': {
                    'response_time_ms': response_time, 'monitoring': 'operational'}}
    ...
    def _output_text_results(self, results):
        ...
        for check_name, check_result in results['checks'].items():
            ...
            self.stdout.write(
                f"{check_name.replace('_', ' ').title()}: {status_str}")
